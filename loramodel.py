...
class ScaleSpaceFlowLora(CompressionModel):
    ...
    def __init__(
        ...
        class LoraDecoder(nn.Sequential):
            def __init__(
                self, out_planes: int, in_planes: int = 192, mid_planes: int = 128
            ):
                super().__init__(
                    lora.ConvTranspose2d(in_planes,mid_planes,kernel_size=5,stride=2,output_padding=1,padding=5 // 2,r = 16,lora_alpha= 32),
                    nn.ReLU(inplace=True),
                    lora.ConvTranspose2d(mid_planes,mid_planes,kernel_size=5,stride=2,output_padding=1,padding=5 // 2,r = 8,lora_alpha= 16),
                    nn.ReLU(inplace=True),
                    lora.ConvTranspose2d(mid_planes,mid_planes,kernel_size=5,stride=2,output_padding=1,padding=5 // 2,r = 8,lora_alpha= 16),
                    nn.ReLU(inplace=True),
                    lora.ConvTranspose2d(mid_planes,out_planes,kernel_size=5,stride=2,output_padding=1,padding=5 // 2,r = 2,lora_alpha= 4),
                )

        class RepeatDecoder(nn.Sequential):
            def __init__(
                self, out_planes: int, in_planes: int = 192, mid_planes: int = 128
            ):
                super().__init__(
                    lora.ConvTranspose2drepeat(in_planes,mid_planes,kernel_size=5,stride=2,output_padding=1,padding=5 // 2,r = 16,lora_alpha= 32),
                    nn.ReLU(inplace=True),
                    lora.ConvTranspose2drepeat(mid_planes,mid_planes,kernel_size=5,stride=2,output_padding=1,padding=5 // 2,r = 8,lora_alpha= 16),
                    nn.ReLU(inplace=True),
                    lora.ConvTranspose2drepeat(mid_planes,mid_planes,kernel_size=5,stride=2,output_padding=1,padding=5 // 2,r = 8,lora_alpha= 16),
                    nn.ReLU(inplace=True),
                    lora.ConvTranspose2drepeat(mid_planes,out_planes,kernel_size=5,stride=2,output_padding=1,padding=5 // 2,r = 2,lora_alpha= 4),
                )
        ...
        class Hyperprior(CompressionModel):
            ...
            def forward(self, y, training):
                ...
                z_hat, z_likelihoods = self.entropy_bottleneck(z, training)

                scales = self.hyper_decoder_scale(z_hat)
                means = self.hyper_decoder_mean(z_hat)

                _, y_likelihoods = self.gaussian_conditional(y, scales, means, training)
        ...
        if method == "lora": 
            self.img_decoder = LoraDecoder(3)
            self.res_decoder = LoraDecoder(3, in_planes=384)
            self.motion_decoder = LoraDecoder(2 + 1)

        elif method == "repeat": 
            self.img_decoder = RepeatDecoder(3)
            self.res_decoder = RepeatDecoder(3, in_planes=384)
            self.motion_decoder = RepeatDecoder(2 + 1)
    ...
    def forward(self, frames):
        ...
        reconstructions.append(x_hat.clamp_(0, 1))
        frames_likelihoods.append(likelihoods)
        x_ref = x_hat.detach()  # stop gradient flow (cf: google2020 paper)

        for i in range(1, len(frames)):
            x = frames[i]
            x_ref, likelihoods = self.forward_inter(x, x_ref.clamp_(0, 1))
            reconstructions.append(x_ref.clamp_(0, 1))
            frames_likelihoods.append(likelihoods)
    ...
    def forward_keyframe(self, x):
        ...
        y_hat, likelihoods = self.img_hyperprior(y, self.training)
    ...
    def forward_inter(self, x_cur, x_ref):
        ...
        y_motion_hat, motion_likelihoods = self.motion_hyperprior(y_motion,self.training)
        ...
        y_res_hat, res_likelihoods = self.res_hyperprior(y_res,self.training)
    ...
    def compress(self, frames):
        ...
        bits += len(out_keyframe["strings"][0][0])*8
        bits += len(out_keyframe["strings"][1][0])*8

        for i in range(1, len(frames)):
            x = frames[i]
            x_ref, out_interframe = self.encode_inter(x, x_ref.clamp_(0, 1))

            frame_strings.append(out_interframe["strings"])
            shape_infos.append(out_interframe["shape"])

            bits += len(out_interframe["strings"]["motion"][0][0])*8
            bits += len(out_interframe["strings"]["motion"][1][0])*8
            bits += len(out_interframe["strings"]["residual"][0][0])*8
            bits += len(out_interframe["strings"]["residual"][1][0])*8

        return frame_strings, shape_infos, bits
    ...
    def decompress(self, strings, shapes):
        ...
        x_ref = self.decode_keyframe(strings[0], shapes[0]).clamp_(0, 1)
        dec_frames.append(x_ref)

        for i in range(1, len(strings)):
            string = strings[i]
            shape = shapes[i]
            x_ref = self.decode_inter(x_ref, string, shape).clamp_(0, 1)
            dec_frames.append(x_ref)
